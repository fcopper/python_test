...
###############################配置信息################################
# 程序地址
m3u8_downloader_path = "C:/Users/XXS/Downloads/m3u8/m3u8_downloader"
# m3u8链接批量输入文件(必须是utf-8编码)
m3u8InputFilePath = m3u8_downloader_path + "/m3u8_input.txt"
# 设置视频保存路径
saveRootDirPath = "C:/Users/XXS/Downloads"
# 下载出错的m3u8保存文件
errorM3u8InfoDirPath = m3u8_downloader_path + "/error.txt"
...
doneCount = 0
# cache path
cachePath = m3u8_downloader_path + "/cache"
# log path
logPath = m3u8_downloader_path + "/log.log"
# log file
logFile = None
...
def ffmpegConvertToMp4(inputFilePath, ouputFilePath):
    ...
    # C:/Users/XXS/Downloads/m3u8/m3u8_downloader/lib/
    cmd = r'ffmpeg -loglevel quiet -i "{0}" -vcodec copy -acodec copy "{1}"'.format(inputFilePath, ouputFilePath)
    if os.system(cmd) == 0:
        print("\t转换成功！")
        logFile.write("\t转换成功!---------------------\n")
        return True
...
if __name__ == '__main__':
    ...
    lines = m3u8InputFp.readlines()
    new_lines = list(filter(None, (line.rstrip() for line in lines))) #排除掉空行
    totalFile = len(new_lines) #待下载的总文件数
    fileCount = 0 #已经完成的文件数
    errorCount = 0 #下载失败的文件数
    for rowData in new_lines:
        ...
        try:
            fileCount += 1
            print("共{0}个文件，失败{1}个".format(totalFile,errorCount))
            print("\t开始下载第{0}个:【{1}】".format(fileCount,m3u8Info[0]))
            logFile.write("开始下载:{0}\n".format(m3u8Info[0]))
